...
def sliceCBar(maxspeed, cbarname):
    ...
    cmap = matplotlib.colormaps.get_cmap(cbarname, maxspeed)    # PiYG
...
def main():
    ...
    while(1):
        ...
        try:
            ...
            moonaz = "{:.1f}".format((get_body("moon", Time.now()).transform_to(AltAz(obstime=Time.now(), location=elginfield)).az*u.deg).value)
            moonalt = "{:.1f}".format((get_body("moon", Time.now()).transform_to(AltAz(obstime=Time.now(), location=elginfield)).alt*u.deg).value)
            sunaz = "{:.1f}".format((get_body("sun", Time.now()).transform_to(AltAz(obstime=Time.now(), location=elginfield)).az*u.deg).value)
            sunalt = "{:.1f}".format((get_body("sun", Time.now()).transform_to(AltAz(obstime=Time.now(), location=elginfield)).alt*u.deg).value)
            ...
            moon_current = get_body("moon", current_times)
...
def openCloudLog(logname):
    ...
    if not os.path.exists(logname):
        print(f"Error: File {logname} does not exist.")
...
def main():
    ...
    while True:
        try:
            ...
            wx_socket = socket(AF_INET, SOCK_STREAM)
            wx_socket.settimeout(20)
            wx_socket.connect(wx_address)

            polaris_socket = socket(AF_INET, SOCK_STREAM)
            polaris_socket.settimeout(20)
            polaris_socket.connect(polaris_address)
            ...
            try:
                ...
                if polaris_found != 1 or polaris_mag > -13.8 or polaris_darkness >= 37:
                    ...
                    run_state = False
                    start_time = time.time()
                elif elapsed_time > 300 and polaris_mag < -14.0 and polaris_darkness < 34 and not run_state:
                    print('Polaris visible!')
                    cloud_flag = 1
                    alert_flag = 0
                    run_state = True
                    ...
                    print('Polaris is visible. Will start up in %s seconds.' % int(300 - elapsed_time))
                elif elapsed_time < 300 and polaris_mag < -14.0 and polaris_darkness < 34 and run_state:
                    print('Polaris is still visible.')
                elif elapsed_time < 300:
                    print('Polaris is likely visible, but let\'s wait a bit longer...')
                    print('Waiting %s seconds.' % int(300 - elapsed_time))
                ...
                print(f"Error getting Polaris data: {e}")
                cloud_flag = -999
                polaris_flag = 0
            ...
            try:
                print('Trying to get cloud data...')
                cloud_log_file = os.path.abspath(os.path.join("d:/Logs/Weather/CloudMonitor", "current-cloud.log"))
                sky_t, ground_t = getClouds(cloud_log_file)
                delta_t = sky_t - ground_t
                print('Got cloud data! Sky=%s Ground=%s Delta=%s' % (str('%.2f' % sky_t), str('%.2f' % ground_t), str('%.2f' % delta_t)))
                ...
                dew_flag = 0 if dew > 0 else 0
                rain_flag = 3 if rain > 0 else 0
                ...
                log_path = 'd:/Logs/Weather/'
                image_path = 'd:/Logs/Weather/Images/'
                with open(log_path + 'weather-current.log', 'w') as f:
                    fmt = '%10s%9s%3s%2s%2s%7s%7s%7s%7s%4s%7s%4s%2s%2s%6s%13s%2s%2s%2s%2s%2s%2s'
                    f.write(fmt % (datenow, timenow, '.00', 'C', 'K', str('%.1f' % round(sky_t,2)), str('%.1f' % round(ground_t,1)), str(t_obs), str(v_wnd),
                                 str(h_out), str(dew), '000', '0', '0', '0001', '1', str(int(cloud_flag)), '1', str(int(rain_flag)), '0', '0', str(int(alert_flag))))
                ...
                toff = tdec - 24.0 if tdec > 12.0 and tdec < 24.0 else tdec
                ...
                cmaparr = sliceCBar(maxspeed,'viridis')
                cloudblk, cloudx, cloudy, tempg = openCloudLog(cloud_log_file)
                
                if cloudblk is None:
                    raise FileNotFoundError(f"Cloud log file {cloud_log_file} not found.")
                ...
                spdcolor = 'orange' if speed > warnspd & speed < shutspd else 'red' if speed > shutspd else 'green'
                wedge_spd = mpatches.Wedge([0,0], 0.085, spdang(speed), 180, width=0.02, ec="none", fc=spdcolor, lw=1)
                ...
                fontname = 'bold_led_board-7.ttf'

                fontcolor = 'orange' if speed > warnspd & speed < shutspd else 'red' if speed > shutspd else 'green'
                ax_spd.text(0, 0.0115, speed, horizontalalignment='center', verticalalignment='center', fontweight='bold', color=fontcolor, fontproperties=fm.FontProperties(fname=fontname, size=30))
                ...
                print(f"Error getting cloud/weather data: {e}")
