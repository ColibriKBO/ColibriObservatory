...
function writeRequestsToCSV(requests, debug) {
    try {
        ...
        if (debug === undefined) { // normal operation of code
            ...
            var header = "Directory Name,Priority,RA,Dec,Start Time (UTC),End Time (UTC),Obs Duration (mins),Exposure Time (seconds),Filter,Binning,Total Score"
            writeFile.WriteLine(header); // Write header to the file
        }
        else{
            // debug mode which can be tested in say VS Code
            fs = require('fs');
            data = "Directory Name,Priority,RA,Dec,Start Time (UTC),End Time (UTC),Obs Duration (mins),Exposure Time (seconds),Filter,Binning,Total Score\n";
        }
