...
parent_dir = r"D:\colibrigrab_test_organized"
...
def organize_by_exposure(parent_dir, subdirs):
    exposure_dict = {}
    for subdir in subdirs:
        ...
        if len(parts) > 1:
            exposure = parts[1]
            exposure_dir = os.path.join(parent_dir, exposure + "ms")
...
def move_time_dirs_to_root_and_cleanup(exposure_dict):
    for exposure_dir in exposure_dict.values():
        ...
        for time_dir in time_dirs:
            ...
            new_time_dir_path = os.path.join(parent_dir, os.path.basename(exposure_dir), new_time_dir_name)
...
exposure_dict = organize_by_exposure(parent_dir, parent_subdirs)
