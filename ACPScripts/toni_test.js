...
function adjustPointing(ra, dec)
{
    ...
    while(BS.Status != 1)
    {
        ...
        Util.WaitForMilliseconds(100000);
    };
...
// Defina o número total de capturas desejadas
var totalCaptures = 300;

// Loop até atingir o número total de capturas
for (var i = 0; i < totalCaptures; i++) {
    // Slew to coordinates
    //gotoRADec(RA, DEC);

    // Adjust pointing
    //for (var j = 0; j < numCorrections; j++) {
    //    adjustPointing(RA, DEC);
    //}

    Console.PrintLine("Dome is done slewing and telescope is at target.");
    Console.PrintLine("Collecting darks...");
    var captureCount = i + 1; // Para exibir a contagem da captura
    pid = Util.ShellExec("ColibriGrab.exe", "-n 2400 -p LP_89_187 -e 30000 -t 0 -f dark + -w D:\\Ultra_Cool_Dwarfs_Project\\2024-04-05\\");
    Console.PrintLine("Process ID = " + pid.toString());
    Util.WaitForMilliseconds(5000000);
    ...
    // Atualize weather_check aqui, assumindo que será atualizado em outro lugar no código
    // Caso contrário, você pode enfrentar um loop infinito
}}
